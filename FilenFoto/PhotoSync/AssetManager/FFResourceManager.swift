...
class FFResourceManager {
    ...
    func fetchAssets(for parentContextFotoAsset: FFObjectID<FotoAsset>, from iosPhotoAsset: PHAsset, writeTo destinationFolder: URL) async throws
        -> WorkingAssetState
    {
        return try await withTemporaryManagedObjectContext(parentContextFotoAsset) { fotoAsset, objectContext in
            ...
            if existingRemoteResourcesCount > 0 {
                let wasAssetModified = iosPhotoAsset.modificationDate != fotoAsset.dateModified
    ...
    func syncResources(in workingDirectory: URL, for fotoAssetId: FFObjectID<FotoAsset>) async throws {
        try await withTemporaryManagedObjectContext (fotoAssetId) { fotoAsset in
            ...
            for resourceToUpload in stagedRemoteResourcesToUpload {
                try await filenUpload(resource: typedID(resourceToUpload), inCloudFolder: filenResourceFolderUuidString, inLocalFolder: workingDirectory)
            }
            
            for resourceToDeleteParentContext in stagedRemoteResourcesToDelete {
                try await filenDelete(resource: typedID(resourceToDeleteParentContext), inLocalFolder: workingDirectory)
            }
        }
    }
