...
public struct PageView<Content: View>: View {
    ...
    @State var displayedItemId: Int? = nil {
        didSet {
            if let displayedItemId {
                pageIndex.wrappedValue = displayedItemId
            }
        }
    }
    
    @State var displayedItems: [Int] = []
    ...
    private func calculateDisplayItems(originalIndex: Int) {
        ...
        for i in start...end {
            if let index = items.index(items.startIndex, offsetBy: i, limitedBy: items.endIndex) {
                displayedItems.append(index)
            }
        }
    }
    
    public var body: some View {
        let _ = print(disablePaging.wrappedValue)
        #if !os(tvOS)
        ...
        #if !os(tvOS)
        .simultaneousGesture(disablePaging.wrappedValue ? nil : gesture, including: disablePaging.wrappedValue ? .subviews : .all)
        #endif
        .onAppear {
    ...
    private func updateVirtualPageIndex(originalIndex: Int) {
        ...
        displayedItemId = displayedItems[Int(virtualPageIndex)]
    }
}
